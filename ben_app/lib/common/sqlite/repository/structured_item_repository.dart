...
class StructuredItemRepository
    extends SqliteRepository<String, StructuredItemEntity> {
  ...
  Future<StructuredItemEntity> getByIdAndType(String id, int type) async {
    final List<Map<String, dynamic>> results = await db
        .query(tableName, where: "id=? and type=?", whereArgs: [id, type]);
  ...
  Future<List<MetaView>> getItemsByType(int type) async {
    ...
    final List<MetaView> items =
        results.map((values) => MetaView.from(values)).toList();
    return items;
  }
}
